...
`load_eval_samples(..., strategy="stratified")`로 쓸 수 있다. 라이브러리 기본값은 `"head"`로
두었다 — 조용히 바꾸면 `RESULTS.md`와 리더보드의 기존 수치가 무효가 되기 때문이다. 리더보드는
대신 **두 슬라이스를 모두 돌려 나란히 보고**한다 (§5 D1).
