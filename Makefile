.PHONY: venv install install-gpu install-all install-vllm install-llama install-webui \
...
# Lockfile
# ---------------------------------------------------------------------------
# `uv sync` reproduces uv.lock; these two are how the lock itself changes.
...
# Re-resolve within the pyproject constraints.
upgrade:
	uv lock --upgrade
